...
% %skteching : je zappe ca
% In that view, it is related to sketching methods \cite{jesaispas} sketching \cite{gower2018stochastic} although in our case the random subspaces of evaluation is not taken uniformly but along chosen structure adapted to the regularizer. -> Sec. 2.4 (top)

% In the context of  $\ell_1$-regularized problems, they can be combined with screening techniques for efficiently solving high-dimensional problems, \cite{Massias_Gramfort_Salmon18}. -> 2.4.1? % pas besoin ? on cite deja les autres

% 
% adapting coordinate descent probabilities to the identified important \cite{preprint-mitya?}.  -> ???


% more identif':
% However, to the exception of \cite{maybe someone}, identification is only harnessed if it concerns the (coordinate) sparsity of the iterates. For a broader class of regularizers, including the total variation ($g(x) = \sum_{i=2}^n |x_{i}-x_{i-1}|$ in 1D) or weighted $\ell_1$-norm ($g(x)=\|Mx\|_1$ with $M$ an $m\times n$ matrix), the identified subspace is not coordinate-sparse but still sparse on some well-chosen subspaces. 