\section{XXX}\label{sec:soda-identiification}
...
\begin{algorithm}[t!]
\caption{\textsc{\SP} on $((\alpha_i),(f_i),\pi)$ with stopping criterion $\mathsf{C}$}
\label{alg:soda}

\tcbset{width=0.58\columnwidth,before=,after=\hfill, colframe=black,colback=white, fonttitle=\bfseries, coltitle=white, colbacktitle=black, boxrule=0.2mm,arc=0mm, left = 2pt}
...
\WHILE{not interrupted}{
    ...
    \STATE Draw sparsity mask $\mask$ as
    \vspace*{-6pt}
    \begin{equation*}
        \mathbb{P}[j\in\mask] = \begin{cases}
        1,& \text{ if } j\in\supp({x})\\
        \pi,& \text{ if } j\notin\supp({x})
        \end{cases}
    \end{equation*}
    \vspace*{-12pt}
    \STATE $x^+_i = [  x - \gamma \nabla f_i(x)]_{\mask} + [x_i]_{\overline{\mask}}$
    \vspace*{2pt}
...
\end{algorithmic}
\end{tcolorbox}
\tcbset{width=0.38\columnwidth,before=,after=\hfill, colframe=black,colback=white, fonttitle=\bfseries, coltitle=white, colbacktitle=black, boxrule=0.2mm,arc=0mm, left = 2pt}
\begin{tcolorbox}[title=\textsc{Master}
\vphantom{\texttt{Worker  i}}]
~\\\begin{algorithmic}
\STATE \textbf{Initialize} $\barx^0$
%Send $\overline x$ to each machine\\
\WHILE{not converged}{
     \STATE {\color{blue!70!black}Receive\,$\Delta^k$\,from\,agent\,$i^k$}
     \STATE $\barx^k = \barx^{k-1} + \alpha_{i^k}\Delta^k$
    \STATE $ x^k = \prox_{\gamma \lambda\|\cdot\|}(\barx^k )$
    \vspace*{1pt}
    \STATE {\color{red!80!yellow} Send $x^k$ to agent $i^k$}
    \vspace*{2pt}
    %$k\leftarrow k+1$
}
\ENDWHILE
...
Let us recall the theoretical result (see Theorem \ref{lm:spy_diff}), specified for this selection.
\begin{corollary}[Expected deviation]\label{cor:spydr_rate}
...
\end{align}
%so that there are at least two updates of each worker between $k_{m+1}$ and $k_m$.
and 
$$
